...
def test_simple() -> None:
    a_list: RangedList = RangedList(5, 12, "twelve")
...
def test_complex() -> None:
    a_list: RangedList = RangedList(5, 20, "twenty")
...
def test_plus_minus() -> None:
    ...
    d_list: RangedList = RangedList(5, 8, "eight")
    e_list = a_list + d_list
    f_list = e_list - d_list
    assert a_list == f_list


def test_multi_divide() -> None:
    a_list: RangedList = RangedList(5, 12, "twelve")
    b_list = a_list * 2
    c_list = b_list / 2
    assert a_list == c_list
    d_list: RangedList = RangedList(5, 4, "four")
    e_list = a_list / d_list
    f_list = e_list * d_list
    assert a_list == f_list
