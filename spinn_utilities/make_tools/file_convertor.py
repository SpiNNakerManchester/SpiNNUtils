...
class FileConvertor(object):
    ...
    def __init__(self, src, dest, csv):
        self._src = src
        self._dest = dest
        self._csv = csv
    ...
    def run(self):
        ...
        self._message_id = self.get_message_id(src)
        self.convert_c()
    ...
    def get_message_id(self, src, dir=None):
        if dir is None:
            dir = os.path.dirname(src)
        else:
            dir = os.path.dirname(dir)
        log_ranges_path = os.path.join(dir, "log_ranges.txt")
        if not os.path.exists(log_ranges_path):
            if dir == src:
                raise Exception(
                    "No log_ranges.txt file found for {}".format(src))
            return self.get_message_id(src, dir)
        ...
        with open(log_ranges_path, 'r') as log_ranges_file:
            ...
            for line in data_lines:
                ...
                if filename.strip() == parts[1].strip():
                    return int(parts[0])
        raise Exception("{} not found in {}".format(filename, log_ranges_path))

    @staticmethod
    def convert(src, dest, csv):
        convertor = FileConvertor(src, dest, csv)
        convertor.run()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 3:
        ...
        csv = src + ".csv"
    FileConvertor.convert(src, dest, csv)
